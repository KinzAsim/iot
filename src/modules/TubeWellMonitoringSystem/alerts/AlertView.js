...
import { getTubeWellAlerts } from '../../../redux/actions/tubewellAction';
...
const AlertScreen = props => {
  ...
  const alert = useSelector(state => state.tubewell.alerts);
  const AlertLoading = useSelector(state => state.tubewell.alertLoading);

  const fetchAlertsData = async () => {
    await dispatch(getTubeWellAlerts(user.id));
  };
